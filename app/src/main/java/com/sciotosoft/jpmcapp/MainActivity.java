...
public class MainActivity extends AppCompatActivity {
    //Time Permitting, I would have created an additional Fragment for the Max Range problem,
    // leaving MainActivity empty of its logic.
    //todo: build secondary fragment Intent 1hr
    //todo: build secondary fragment 3hrs
    //todo: debug 2hrs
    ...
    private ActionCase runTheNumbers(@NotNull List<Integer> input) {
        ...
        int entryDay = 0;
        int exitDay = 0;
        for (int exteriorIndex : input) {
            //ActionCase bestROI = new ActionCase(0,0,0); //ROI = return on investment
            entryDay = input.get(exteriorIndex); //debug var
            entryDayGain = input.get(exteriorIndex).intValue();
            System.out.println("Entry day gain " + entryDayGain);
            for (int interiorIndex : input) {
                //We cannot sell our stock before we've bought it!! Ergo . . .
                if (interiorIndex < exteriorIndex) {
                    exitDay = input.get(interiorIndex); //debug var
                    exitDayGain = input.get(interiorIndex).intValue();
